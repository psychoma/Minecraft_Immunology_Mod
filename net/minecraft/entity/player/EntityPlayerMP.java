...
public class EntityPlayerMP extends EntityPlayer implements ICrafting
{
    ...
    {
    	super.onFinishedDisease(par1Disease);
    	ByteArrayOutputStream bos = new ByteArrayOutputStream(4);
		DataOutputStream outputStream = new DataOutputStream(bos);
		try{
			outputStream.writeInt(par1Disease.getdiseaseID());
		} catch (Exception ex){
			ex.printStackTrace();
		}
		
		Packet250CustomPayload packet = new Packet250CustomPayload();
		packet.channel = "RemoveDisease";
		packet.data = bos.toByteArray();
		packet.length = bos.size();
    	PacketDispatcher.sendPacketToPlayer(packet, (Player) this);
    }
    protected void onChangedDisease(Disease par1Disease)
    {
    	super.onChangedDisease(par1Disease);
    	PacketDispatcher.sendPacketToPlayer(this.getDiseasePacket(par1Disease), (Player) this);
    }
    protected void onNewDisease(Disease par1Disease)
    {
    	super.onNewDisease(par1Disease);
    	PacketDispatcher.sendPacketToPlayer(this.getDiseasePacket(par1Disease), (Player) this);	
    }
    private Packet250CustomPayload getDiseasePacket(Disease par1Disease)
    {
    	ByteArrayOutputStream bos = new ByteArrayOutputStream(12);
		DataOutputStream outputStream = new DataOutputStream(bos);
		try{
			outputStream.writeInt(par1Disease.getdiseaseID());
			outputStream.writeInt(par1Disease.getStage());
			outputStream.writeInt(par1Disease.getDuration());
			outputStream.writeInt(par1Disease.DiseaseEffects.size());
			for(int index = 0; index < par1Disease.DiseaseEffects.size(); index++)
			{
				outputStream.writeInt(par1Disease.DiseaseEffects.get(index));
			}
		} catch (Exception ex){
			ex.printStackTrace();
		}
